...
import { Shield, Activity, Key, Globe, Plus, Trash2, RefreshCw } from "lucide-react";

export const GuardianDemo: React.FC = () => {
    const { isConnected, status, isLoading, ping } = useGuardian();

    const jobsHook = useGuardianJobs();
    const jobsData = jobsHook.jobs.data as z.infer<typeof ListJobsResponseSchema> | undefined;

    const capsHook = useGuardianCapabilities();
    const capsData = capsHook.capabilities.data as z.infer<typeof ListCapabilitiesResponseSchema> | undefined;

    const firewallHook = useGuardianFirewall();
    const rulesData = firewallHook.rules.data as z.infer<typeof ListWfpRulesResponseSchema> | undefined;

    const { createJob, terminateJob } = jobsHook;
    const { requestCapability, revokeCapability } = capsHook;
    const { createRule, deleteRule } = firewallHook;
    ...
    return (
        <div className="p-6 space-y-6 max-w-6xl mx-auto" data-testid="guardian-page">
            <header className="flex justify-between items-center bg-background/50 backdrop-blur-md p-4 rounded-xl border">
                <div>
                    ...
                    <p className="text-muted-foreground">Native Process Isolation & Capability Management</p>
                </div>
            ...
            <Tabs defaultValue="jobs" className="w-full">
                <TabsList className="grid w-full grid-cols-4 mb-8">
                    <TabsTrigger value="jobs" className="flex gap-2" data-testid="tab-jobs">
                        <Activity className="w-4 h-4" /> Job Objects
                    </TabsTrigger>
                    <TabsTrigger value="tokens" className="flex gap-2" data-testid="tab-tokens">
                        <Key className="w-4 h-4" /> Capabilities
                    </TabsTrigger>
                    <TabsTrigger value="firewall" className="flex gap-2" data-testid="tab-firewall">
                        <Globe className="w-4 h-4" /> Firewall
                    </TabsTrigger>
                    <TabsTrigger value="status" className="flex gap-2" data-testid="tab-status">
                                        ...
                                        >
                                            <div className="flex items-center gap-3">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="text-xs text-muted-foreground">Process Isolation Layer</p>
                                                </div>
                                            </div>
                                            <Button 
                                                variant="destructive" 
                                                size="sm" 
                                                onClick={() => terminateJob.mutate({ jobName })} 
                ...
                <TabsContent value="tokens">
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        ...
                        <Card className="p-4 md-col-span-2">
                            <h3 className="font-semibold mb-4 text-lg">Capability Inventory</h3>
                            <div className="space-y-3">
                                            ...
                                            <div className="flex justify-between items-start">
                                                <div className="flex gap-3">
                                                    ...
                                                    <div>
                                                        ...
                                                        <p className="text-xs text-muted-foreground">Action: <span className="text-primary uppercase font-bold">{cap.action}</span></p>
                                                    </div>
                                                </div>
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {!cap.revoked && (
                                                        <Button 
                                                            ...
                                                            onClick={() => revokeCapability.mutate({ tokenId: cap.tokenId })}
                    ...
                    <Card className="p-8 text-center border-dashed border-2">
                        ...
                        <div className="grid grid-cols-1 md-grid-cols-2 gap-4 max-w-2xl mx-auto">
                            {rulesData?.rules?.map((rule: any) => (
                                <div key={rule.ruleId} className="p-4 border rounded-xl text-left flex justify-between items-center">
                                    <div>
                                        <p className="font-semibold">{rule.name}</p>
                                        <p className="text-xs text-muted-foreground">{rule.direction} | {rule.action}</p>
                                    </div>
                                    <Button variant="ghost" size="icon" onClick={() => deleteRule.mutate({ ruleId: rule.ruleId })}>
                                        <Trash2 className="w-4 h-4 h-destructive" />
                                    </Button>
                                </div>
                            ))}
                        </div>
                        <p className="text-sm italic mt-6">WFP Manager is currently in stub mode. Fully native implementation pending.</p>
                    </Card>
                </TabsContent>
                ...
                <TabsContent value="status">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <Card className="p-6">
                            ...
                            <div className="space-y-4">
                                <div className="flex justify-between border-b pb-2">
                                    ...
                                    <code className="bg-accent px-1 rounded">\\.\pipe\DyadGuardian</code>
                                </div>
                                <div className="flex justify-between border-b pb-2">
                                ...
                                <div className="flex justify-between border-b pb-2">
                                    ...
                                    <span>{(status.data as any)?.timestamp ? new Date((status.data as any).timestamp).toLocaleTimeString() : "N/A"}</span>
                                </div>
                        ...
                        <Card className="p-6 bg-primary/5 border-primary/20">
                            <h3 className="font-semibold mb-4 text-primary">Security Architecture</h3>
                            <ul className="space-y-3 text-sm">
                                <li className="flex gap-2">
                                    <Shield className="w-4 h-4 text-primary shrink-0" />
                                    <span><strong>Kernel Objects:</strong> Jobs are used to hard-restrict memory and CPU cycles.</span>
                                </li>
                                <li className="flex gap-2">
                                    <Shield className="w-4 h-4 text-primary shrink-0" />
                                    <span><strong>Zero-Trust:</strong> Every request requires a valid JWT Capability Token.</span>
                                </li>
                                <li className="flex gap-2">
                                    <Shield className="w-4 h-4 text-primary shrink-0" />
                                    <span><strong>Process Shield:</strong> Child processes automatically inherit parent job restrictions.</span>
                                </li>
                            </ul>
                        </Card>
                    </div>
