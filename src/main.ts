...
import { handleExactaProReturn } from "./main/pro";
...
if (process.defaultApp) {
  if (process.argv.length >= 2) {
    app.setAsDefaultProtocolClient("exacta", process.execPath, [
      path.resolve(process.argv[1]),
    ]);
  }
} else {
  app.setAsDefaultProtocolClient("exacta");
}
...
export async function onReady() {
  ...
  if (settings.enableAutoUpdate) {
    ...
    const host = `https://api.exacta.ai/v1/update/${postfix}`;
    logger.info("Auto-update release channel=", postfix);
    updateElectronApp({
      logger,
      updateSource: {
        type: UpdateSourceType.ElectronPublicUpdateService,
        repo: "pavan53732/Exacta-App-Studio",
        host,
      },
...
async function handleDeepLinkReturn(url: string) {
  ...
  if (parsed.protocol !== "exacta:") {
    dialog.showErrorBox(
      "Invalid Protocol",
      `Expected exacta://, got ${parsed.protocol}. Full URL: ${url}`,
    );
    return;
  }
  ...
  if (parsed.hostname === "exacta-pro-return") {
    const apiKey = parsed.searchParams.get("key");
    if (!apiKey) {
      dialog.showErrorBox("Invalid URL", "Expected key");
      return;
    }
    handleExactaProReturn({
      apiKey,
    });
    // Send message to renderer to trigger re-render
    mainWindow?.webContents.send("deep-link-received", {
