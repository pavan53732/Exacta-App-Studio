...
export class AIDebugger extends EventEmitter {
  ...
  recordEvent(type: DebugEvent['type'], content: any, metadata?: Record<string, any>): string {
  ...
  recordConversationTurn(turn: Omit<ConversationTurn, 'id' | 'timestamp'>): string {
    ...
    logger.info(`Recorded conversation turn: ${turnId} (${turn.success ? 'success' : 'failed'})`);
  ...
  searchEvents(query: string): DebugEvent[] {
    return this.debugEvents.filter(event => 
      JSON.stringify(event.content).toLowerCase().includes(query.toLowerCase()) ||
      JSON.stringify(event.metadata).toLowerCase().includes(query.toLowerCase())
    );
  }
  ...
  getStatistics(): {
    ...
    const successfulConversations = this.conversationHistory.filter(c => c.success).length;
    const totalProcessingTime = this.conversationHistory.reduce((sum, c) => sum + c.processingTime, 0);
    const averageProcessingTime = this.conversationHistory.length > 0 
...
export const debugHelpers = {
  createConversationContext(userId: string, appId: number, chatId: number): any {
    return {
      userId,
      appId,
      chatId,
