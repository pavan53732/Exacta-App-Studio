...
export class PerformanceMonitor extends EventEmitter {
  ...
  private baselineMetrics: Map<string, { avgDuration: number; count: number }> = new Map();
  ...
  private recordMetric(metric: PerformanceMetrics): void {
    ...
    logger.debug(`Performance recorded: ${metric.operation} took ${metric.duration}ms`);
  }

  private updateBaseline(metric: PerformanceMetrics): void {
    const baseline = this.baselineMetrics.get(metric.operation);
    if (baseline) {
      const newAvg = (baseline.avgDuration * baseline.count + metric.duration) / (baseline.count + 1);
  ...
  private checkPerformanceAlerts(metric: PerformanceMetrics): void {
    ...
    if (metric.duration > this.thresholds.slowOperation) {
      alerts.push(`Slow operation: ${metric.operation} took ${metric.duration}ms (threshold: ${this.thresholds.slowOperation}ms)`);
    }

    // Check for high memory usage
    if (metric.memoryUsage && metric.memoryUsage.heapUsed > this.thresholds.highMemory) {
      alerts.push(`High memory usage: ${Math.round(metric.memoryUsage.heapUsed / 1024 / 1024)}MB (threshold: ${this.thresholds.highMemory / 1024 / 1024}MB)`);
    }

    // Check performance degradation compared to baseline
    const baseline = this.baselineMetrics.get(metric.operation);
    if (baseline && baseline.count > 5) { // Need enough samples
      const degradation = ((metric.duration - baseline.avgDuration) / baseline.avgDuration) * 100;
      if (degradation > this.thresholds.warningThreshold) {
        alerts.push(`Performance degradation: ${metric.operation} is ${degradation.toFixed(1)}% slower than baseline (${baseline.avgDuration.toFixed(1)}ms)`);
      }
    }
  ...
  getSlowOperations(thresholdMs: number = this.thresholds.slowOperation): PerformanceMetrics[] {
    return this.metrics.filter(m => m.duration > thresholdMs && m.success);
  }

  getPerformanceTrends(operation: string, timeWindowHours: number = 24): {
    ...
    // Group by hour
    const hourlyData = new Map<number, { durations: number[]; count: number }>();
    
    recentMetrics.forEach(metric => {
      const hour = Math.floor((metric.timestamp - cutoffTime) / (60 * 60 * 1000));
    ...
    if (hourlyAverages.length >= 2) {
      ...
      const firstAvg = firstHalf.reduce((sum, h) => sum + h.avgDuration, 0) / firstHalf.length;
      const secondAvg = secondHalf.reduce((sum, h) => sum + h.avgDuration, 0) / secondHalf.length;
  ...
  suggestMemoryOptimizations(): string[] {
    ...
    if (currentMemory.heapUsed > this.thresholds.highMemory * 0.8) {
      ...
      suggestions.push("Review file processing to use streams instead of loading entire files");
      suggestions.push("Implement proper cleanup of large objects and event listeners");
    }

    // Check for memory leaks in metrics
    if (this.metrics.length > this.MAX_METRICS * 0.9) {
      suggestions.push("Performance metrics buffer nearly full - consider reducing retention");
    }
...
export function PerformanceTracked(operationName?: string) {
  return function(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = async function(...args: any[]) {
      ...
      return perfMonitor.measureOperation(opName, () => originalMethod.apply(this, args), {
