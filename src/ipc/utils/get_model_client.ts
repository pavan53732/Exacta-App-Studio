...
export async function getModelClient(
  ...
  // Handle Enhanced Mode override
  if (dyadApiKey && settings.enableDyadPro) {
    // Check if the selected provider supports Enhanced Mode (has a gateway prefix) OR
    ...
    if (providerConfig.gatewayPrefix != null || dyadEngineUrl) {
      const provider = createDyadEngine({
        ...
        dyadOptions: {
          ...
          enableSmartFilesContext: true, // Forced Smart Context
      ...
      logger.info(
        `\x1b[1;97;44m Using Enhanced Mode for model: ${model.name} \x1b[0m`,
      );

      logger.info(
        `\x1b[1;30;42m Using LLM engine: ${dyadEngineUrl ?? "<prod>"} \x1b[0m`,
      );
      ...
      return {
        ...
        isSmartContextEnabled: true, // Forced Smart Context
      };
    } else {
      logger.warn(
        `Enhanced Mode enabled, but provider ${model.provider} does not have a gateway prefix defined. Falling back to direct provider connection.`,
      );
