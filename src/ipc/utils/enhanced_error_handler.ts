      ...
      for (const strategy of this.recoveryStrategies) {
        if (strategy.canRecover(enhancedError)) {
          try {
            ...
            if (recovered) {
              logger.info(
                `${operationName} recovered successfully using strategy: ${strategy.getRecoveryMessage(enhancedError)}`,
              );
              return { error: enhancedError, recovered: true };
            }
          } catch (recoveryError) {
            logger.warn(
              `Recovery attempt failed for ${operationName}:`,
              recoveryError,
            );
          }
        }
  ...
  private enhanceError(
    ...
    if (
      ...
      enhancedError.recoverySuggestion =
        "Check if the file path is correct or recreate the missing file";
    } else if (
      ...
      enhancedError.recoverySuggestion =
        "Fix the syntax error in the generated code";
    } else {
...
export class FileNotFoundRecovery implements ErrorRecoveryStrategy {
  ...
  getRecoveryMessage(_error: EnhancedError): string {
  ...
  async recover(error: EnhancedError, context: any): Promise<boolean> {
    if (context.packageName) {
      try {
        ...
        logger.info(
          `Attempting to install missing package: ${context.packageName}`,
        );
        ...
        return new Promise((resolve) => {
          child.on("close", (code) => {
            if (code === 0) {
              ...
              logger.warn(
                `Failed to install ${context.packageName}, exit code: ${code}`,
              );
              resolve(false);
            }
          });
