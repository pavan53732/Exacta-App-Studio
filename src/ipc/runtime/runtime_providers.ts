import { executionKernel, KernelCommand, KernelOptions } from './execution_kernel';
...
export abstract class RuntimeProvider {
  ...
  abstract startDevServer(context: RuntimeContext): Promise<{ url: string; pid: number }>;
  ...
  protected async executeCommand(command: string, args: string[], options: KernelOptions): Promise<any> {
    const kernelCommand: KernelCommand = { command, args };
    return this.kernel.execute(kernelCommand, options);
  }
  
  protected async executeSequence(commands: KernelCommand[], options: KernelOptions): Promise<any[]> {
    return this.kernel.executeSequence(commands, options);
  }
  
  protected logAction(action: string, context: RuntimeContext): void {
    logger.info(`[${this.name}] ${action} for app ${context.appId} at ${context.appPath}`);
  }
}
...
export class NodeRuntimeProvider extends RuntimeProvider {
  ...
  async startDevServer(context: RuntimeContext): Promise<{ url: string; pid: number }> {
    ...
    // Start dev server
    const result = await this.executeCommand('npm', ['run', 'dev', '--', '--port', port.toString()], options);
  ...
  async stopDevServer(pid: number): Promise<void> {
    ...
    try {
      await this.executeCommand('taskkill', ['/PID', pid.toString(), '/F'], options);
    } catch (error) {
...
export class DotNetRuntimeProvider extends RuntimeProvider {
  ...
  async build(context: RuntimeContext): Promise<BuildResult> {
    ...
    try {
      ...
      await this.executeCommand('dotnet', ['build', '--configuration', 'Release'], options);
  ...
  async startDevServer(context: RuntimeContext): Promise<{ url: string; pid: number }> {
    ...
    // Restore and run
    await this.executeCommand('dotnet', ['restore'], options);
    const result = await this.executeCommand('dotnet', ['run', '--urls', `http://localhost:${port}`], options);
  ...
  async stopDevServer(pid: number): Promise<void> {
    this.logAction(`Stopping .NET server PID ${pid}`, { appId: 0, appPath: '' });
  ...
  async package(context: RuntimeContext): Promise<PackageResult> {
    ...
    try {
      await this.executeCommand('dotnet', ['publish', '--configuration', 'Release', '--output', 'publish'], options);
