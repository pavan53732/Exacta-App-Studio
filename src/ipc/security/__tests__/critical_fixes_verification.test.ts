...
describe('ExecutionKernel Critical Fixes Verification', () => {
  ...
  test('should have real getDirectorySize implementation', () => {
    ...
    expect(kernelInstance.getDirectorySize.constructor.name).toBe('AsyncFunction');
  ...
  test('should integrate environment variables', async () => {
    ...
    try {
      ...
      expect(errorMessage).not.toMatch(/spawnControlled|validateCommand|method not found/);
    }
  });
