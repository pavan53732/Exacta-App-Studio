...
describe("safeJoin", () => {
  ...
  describe("safe paths", () => {
    it("should join simple relative paths", () => {
      ...
      // safeJoin normalizes output to forward slashes for cross-platform consistency
    ...
    it("should work with actual temp directory", () => {
      ...
      // safeJoin normalizes output to forward slashes for cross-platform consistency
      expect(result).toBe(tempDir.replace(/\\/g, "/") + "/test/file.txt");
    });
  ...
  describe("edge cases", () => {
    ...
    it("should handle current directory references safely", () => {
      const result = safeJoin(testBaseDir, "./src/file.txt");
      // safeJoin normalizes output to forward slashes
      expect(result).toBe("/app/workspace/src/file.txt");
    });
    ...
    it("should handle very long paths safely", () => {
      ...
      // safeJoin normalizes output to forward slashes
      const expectedPath = "/app/workspace/" + longPath;
