...
- **Task 4: Capability Authority (in Guardian)**
  ...
  - **Sub-task 4.4: Capability Token Details**
    - Implement HMAC-SHA256 signing using the `Guardian_Secret` for all tokens.
    - Implement **Action Sequence Numbering** (monotonic per Goal) to ensure audit trail integrity.
    - Implement strict token validation and expiration logic.
...
- **Task 5: Project Indexer Implementation**
  ...
  - **Sub-task 5.7: Embedding Configuration**
    - Configure the embedding model (target: `text-embedding-3-small` or compatible local equivalent).
    - Implement chunking command: Function/Class boundaries for code, max 512 tokens for text.
...
- **Task 9: Action Execution Engine (in Core & Guardian)**
  ...
  - **Sub-task 9.5: CLI Agent Support (CLI_AGENT_EXEC)**
    - Implement binary hash verification against the signed registry.
    - Implement argument whitelisting for recognized agents (Aider, GPT Engineer, Goose, OpenCode, Blackbox, Crush, Codex, Gemini).
    - Implement logic to link CLI agents to their parent cloud provider credentials.
  - **Sub-task 9.6: Package Manager Support (PACKAGE_EXEC)**
    - Implement secure wrappers for **NuGet**, **npm**, and **pip** integration.
    - Implement **Chocolatey** integration (requiring SHELL_EXEC permissions).
...
- **Task 10: State & Recovery**
  ...
  - **Sub-task 10.4: Budget & Limits Implementation**
    - Implement Token counters (Soft limit: 1M, Hard limit: 10M).
    - Implement File/Line counters (Soft: 100 files/5000 lines).
    - Implement Time limits (Soft: 60min, Hard: 4hr) and Disk limits (10GB).
...
- **Task 11: Operator UI (Electron Project)**
  ...
  - **Sub-task 11.2:** The UI will communicate _only_ with the `Core` process via a standard IPC mechanism (e.g., websockets).
...
- **Task 13: Security Validation**
  ...
  - **Sub-task 13.3: Complete Test Suite Implementation**
    - Implement Sandbox tests: **SBX-001** to **SBX-007**.
