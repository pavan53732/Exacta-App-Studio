...
- **INV-OPS-5: No Retry on Security Class** *(New in V2.2)* — Any error with prefix `SEC-` SHALL immediately abort the operation, disable Smart Retry, and escalate to the operator. Security failures are **never retryable**.

**Related:** Error System (F2), Smart Retry (E6, F3), Safe Mode (F7)