...
## Error Escalation Policy *(New in V2.2)*

Critical errors trigger automatic system state transitions:

| Code Pattern | Escalation Action | Rationale |
| --- | --- | --- |
| **SEC-xxx** | Enter Safe Mode (F7) | Security violations indicate potential compromise or policy breach |
| **IPC-xxx** | Restart Guardian | Protocol failures may indicate Core/Guardian desync |
| **FS-xxx (3x)** | Enter Safe Mode | Repeated filesystem failures suggest environmental instability |
| **AI-xxx (3x)** | Abort + require user | Provider failures exhaust retry budget; human decision required |

**Enforcement:** See INV-OPS-5 (F1) for security error handling invariant.

**Related:** Safe Mode (F7), Resilience Invariants (F1), Self-Healing (F3)