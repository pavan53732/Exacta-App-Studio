...
- **INV-SHELL-7: System Path Denial** — Shell commands MUST be denied if the resolved target path intersects any SYSTEM-OWNED path defined by INV-GLOBAL-15, regardless of profile mode, allowlist status, or user approval. System paths include `.exacta/`, `C:\\ProgramData\\Exacta\\`, Guardian installation, trust store, policy root, upgrade verification, and rollback archive directories.
- **INV-SHELL-8: Deterministic Classification** — All shell commands MUST resolve to an explicit ALLOW, DENY, or CONFIRM class. "Unknown" SHALL NOT map to ALLOW under any profile.
...
| **PROFILE-FULL-AUTO** | ✅ Enabled | Blocklist Only | **CONFIRM** |
