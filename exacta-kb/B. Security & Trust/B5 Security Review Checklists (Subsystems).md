...
- [ ]  Identify all **untrusted inputs** to the subsystem (UI strings from external sources, filesystem content, network responses).
- [ ]  Identify all **trusted inputs** (compile-time constants, validated config, deterministic state, AI outputs after validation).
- [ ]  Confirm the subsystem treats AI output as **trusted advisor output** (validated but not adversarial).
