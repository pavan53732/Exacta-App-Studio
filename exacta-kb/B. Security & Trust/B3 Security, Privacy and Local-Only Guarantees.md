# B3. Security, Privacy and Local-Only Guarantees

This document defines the security model, privacy guarantees, and local-only constraints of the autonomous agent. These are not aspirational goals; they are hard requirements.
...
| Package managers | npm, nuget, etc. | Profile-controlled | Package names, versions |

**No other network calls are made.** If any other network activity is detected, it is a bug.

**V2 Change:** Package manager network access is now enabled by default in PROFILE-DEV and PROFILE-FULL-AUTO, but blocked in PROFILE-SAFE.

**V2.1 Clarification (Safe Mode Override):** When Safe Mode is active, ALL network access is disabled regardless of the settings above. This includes AI APIs, documentation retrieval, and package managers. Safe Mode network restrictions take absolute precedence. See F7. Operator Control & Emergency Recovery for Safe Mode details.
...
**Invariant:** The agent does not auto-add or auto-restore packages without risk-assessed plan.

- Package additions must be part of a generated plan that passes risk assessment
- Package restore is only triggered by explicit build step
- Dependency changes contribute to risk score (high risk requires confirmation)
...
- First preview in a project triggers acknowledgment requirement
...
This is not a dismissible warning; it is a required acknowledgment.
...
| Code injection via AI | Diff validation, syntax checking, risk-based approval |
...
### Shell Execution Security *(New in V2)*

Shell command execution is now enabled by default (in PROFILE-DEV and PROFILE-FULL-AUTO) but strictly controlled:

- **Allowlist/Blocklist:** Commands filtered before execution
- **Sandbox Jail:** Working directory locked to project root
- **Resource Limits:** CPU, memory, time limits enforced
- **Full Audit:** Every command logged with input/output
- **Rollback:** State-modifying commands have rollback strategies

### Self-Upgrade Security *(New in V2)*

The agent can upgrade itself (in PROFILE-FULL-AUTO with explicit config) but:

- **Never Self-Authorizing:** Upgrades flow through standard Gate Pipeline
- **No Hot-Patching:** Cannot modify running binary
- **Atomic:** All-or-nothing with automatic rollback
- **Forbidden Modifications:** Cannot modify security, policy, or audit code
