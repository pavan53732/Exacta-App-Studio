...
3) **Refuse operation** (fail-closed)
- Used when:
...
8. **Diagnostic bundle excludes secrets**
...
10. **IPC payload does not include API key**
    - Make AI call via UI
    - Expected: invoke payload and events never include API key

11. **IPC log events are redacted**
...
12. **Benign `token=` string triggers safe behavior**
