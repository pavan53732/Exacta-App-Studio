...
**Legal Hold Exception** *(New in V2.2)*: AI context preserved under Legal Hold (F8) is classified as forensic evidence and MUST NOT be reloaded into any AI session. Legal Hold storage is write-only for litigation purposes. See INV-MEM-FORENSIC-1 (F8).
...
## Provider Risk Classification *(New in V2.2)*

<aside>
🔒

**INV-AI-1: Provider Classification**

AI providers MUST be classified by trust level. Guardian MUST enforce which provider classes are allowed per autonomy profile and which file classifications may be sent to each class.
...
| --- | --- | --- |
| **LOCAL_ONLY** | No network, no third party | Ollama, LM Studio, local GGUF |
| **TRUSTED_CLOUD** | Contractual privacy guarantees, no training on data | OpenAI API (with DPA), Azure OpenAI |
| **UNTRUSTED_CLOUD** | No guarantees, may log/train | Free tiers, unknown endpoints |

### Enforcement Matrix

| **Provider Class** | **Allowed Data** | **Autonomy Requirement** |
| --- | --- | --- |
| LOCAL_ONLY | Any code, config, logs | Any profile |
| TRUSTED_CLOUD | Code, config (no secrets) | PROFILE-DEV or higher |
| UNTRUSTED_CLOUD | Summaries only, no raw files | PROFILE-FULL-AUTO + explicit opt-in |

### Provider Registration

Guardian maintains provider registry:
...
- Unregistered endpoints default to `UNTRUSTED_CLOUD`
- User must explicitly classify new providers
- Guardian logs all provider classifications

---

## Structural Memory Export Ban *(New in V2.2)*

<aside>
🔒

**INV-AI-2: No Structural Memory Export**

Full project structure metadata MUST NEVER be sent to AI providers. Only task-bounded slices are allowed. This prevents cloud AI from becoming a project memory co-processor.
...
| **Data Type** | **Reason** |  |
| --- | --- | --- |
| Full symbol tables | Project structure exfiltration |  |
| Complete dependency graphs | Architecture disclosure |  |
| Project-wide file indexes | Directory structure leakage |  |
| Context selection rules | Meta-information about what's important |  |
| Historical diff sequences | Change pattern disclosure |  |

### Allowed: Task-Bounded Slices

- Files directly relevant to current task
- Direct dependencies of target file
- User-selected context additions
- Error messages and stack traces (redacted)

---

## Sensitive Data Classification *(New in V2.2)*

<aside>
🔒

**INV-DATA-1: Sensitive Data Classification**

Sensitive data extends beyond secrets. Guardian MUST enforce provider policy against ALL sensitive data classes, not only credentials.
...
| --- | --- | --- |
| **Credentials (Secrets)** | API keys, tokens, passwords, certificates | **NEVER SEND** to any provider |
| **Proprietary Source** | Core business logic, algorithms, trade secrets | Governed by Provider Matrix |
| **Regulated Data** | PII, PHI, PCI, GDPR-protected data | Governed by Provider Matrix |
| **Customer IP** | Client code, customer-specific implementations | Governed by Provider Matrix |
...
interface DataClassification {
  file_path: string;
  detected_classes: ('CREDENTIALS' | 'PROPRIETARY' | 'REGULATED' | 'CUSTOMER_IP')[];
  classification_method: 'pattern' | 'path' | 'annotation' | 'operator_policy';
...
- Credentials detected → BLOCK (no exceptions)
- Other sensitive classes → Check against Provider Classification Matrix
- Unknown classification + UNTRUSTED_CLOUD → BLOCK (fail-closed)
- Operator can override classification per-file via `.exacta/classifications.json`

---

## Provider TLS Enforcement *(New in V2.2)*

<aside>
🔒

**INV-NET-1: Provider TLS Enforcement**

Guardian MUST enforce transport security for all AI provider connections. Insecure connections MUST be blocked.
...
| Certificate Pinning | Optional | SPKI pinning for known providers |
| CA Trust | System CA store | Restricted CA list per provider |
...
- TLS handshake fails → BLOCK + log SEC-501
- Certificate validation fails → BLOCK + log SEC-502
- Pin mismatch → BLOCK + log SEC-503 + alert user
- LOCAL_ONLY providers exempt from TLS requirements

---

## Model Identity Binding *(New in V2.2)*
...
**Rationale:** Prevents forensic invalidation where a plan generated by one model executes under a different model, breaking determinism and legal defensibility.
