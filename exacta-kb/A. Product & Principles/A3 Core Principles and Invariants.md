...
### Revalidation on Resume *(V2.1)*

> On restart or resume, all queued/background tasks MUST be revalidated against:
> 

> - Current Safety Policy hash
> 
...
> Failure → CANCEL + AUDIT.
> 

This ensures that tasks approved under one policy state do not execute under a different (potentially more permissive) policy state.
...
## 11. Autonomy Levels Are UI Presets

**Invariant:** Autonomy Levels (Conservative, Balanced, Aggressive) are **UI presets only**. They do not grant capabilities and cannot exceed **Safety Policy ceilings** enforced by the Guardian.

- UI may present autonomy levels as convenience toggles
- Selected level is capped by `autonomy_ceiling` in Safety Policy
- Attempting to select a level above ceiling is denied and logged
- Levels configure default confirmation behavior within bounds, not authority

**Rationale:** UI convenience must not become a governance bypass vector.
